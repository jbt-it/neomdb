...
import { checkForPermissions } from "../utils/authUtils";
...
export const retrieveMember = (req: Request, res: Response): void => {
  if (Number(req.params.id) === res.locals.memberID || checkForPermissions(res.locals.permissions, 6)) {
    database
      .query(
...
export const updateMember = (req: Request, res: Response): void => {
  ...
  if (Number(req.params.id) === res.locals.memberID && checkForPermissions(res.locals.permissions, 1)) {
    database
      .startTransaction()
  ...
  else if (checkForPermissions(res.locals.permissions, 1)) {
    database
      .query(
        `UPDATE mitglied
