 ...
 */
 ...
 import { ExpandLess, ExpandMore, Edit, AddCircleOutline, Clear, Language as LanguageOfMember } from "@material-ui/icons";
 import {
 ...
 // TODO: Mentor should be only changeable for member with permission 1
 ...
 const DisplayMemberDetails: React.FunctionComponent<DisplayMemberDetailsProps> = (
   ...
   const [mentor, setMentor] = useState<Mentor | null>(memberDetails.mentor ? memberDetails.mentor : null);
               ...
               <Grid item xs={12} sm={12} md={12} lg={12}>
                 <Autocomplete
                   ...
                   onChange={(event, newMentor) => {
                     if (newMentor) {
                       setMentor({mitgliedID: newMentor.mitgliedID, vorname: newMentor.vorname, nachname: newMentor.nachname});
                     }
                   }}
                   defaultValue={members.filter(memb => memb.mitgliedID === mentor?.mitgliedID)[0]}
                           ...
                           <Autocomplete
                              ...
                              onChange={(event, newValue) => {
                                  if (typeof newValue === "string") {
                                    ...
                                    const updatedLanguage: LanguageOfMember = { ...languages[index], wert: newValue};
                                    ...
                                    const list = [...languages];
                                    // Make a new object of the updated language
                                    const updatedLanguage: LanguageOfMember = { ...languages[index], wert: newValue.inputValue};
                                    // Overwrite old value with the new language
                                    list[index] = updatedLanguage;
                                    setLanguages(list);
                                  } else if (newValue) {
                                    const list = [...languages];
                                    // Make a new object of the updated language
                                    const updatedLanguage: LanguageOfMember = { ...languages[index], wert: newValue.wert};
                                    // Overwrite old value with the new language
                                    list[index] = updatedLanguage;
                                    setLanguages(list);
                                  }
                                    ...
                                    <FormControl className={classes.fullWidth}>
                                     ...
                                     <Select
                                       ...
                                       onChange={(event) => {
                                         ...
                                         const updatedLanguage: LanguageOfMember = { ...languages[index], niveau: ""+event.target.value};
                                         // Overwrite old value with the new language
                                         list[index] = updatedLanguage;
                                         setLanguages(list);
                                       }}
