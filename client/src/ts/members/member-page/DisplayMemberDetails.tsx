...
const DisplayMemberDetails: React.FunctionComponent<DisplayMemberDetailsProps> =
  (props: DisplayMemberDetailsProps) => {
    ...
    const [generalInfoDialogOpen, setGeneralInfoDialogOpen] =
      useState<boolean>(false);
    const [clubInfoDialogOpen, setClubInfoDialogOpen] =
      useState<boolean>(false);
    const [studyInfoDialogOpen, setStudyInfoDialogOpen] =
      useState<boolean>(false);
    const [paymentInfoDialogOpen, setPaymentInfoDialogOpen] =
      useState<boolean>(false);
    const [qualificationInfoDialogOpen, setQualificationInfoDialogOpen] =
      useState<boolean>(false);
     ...
     */
    const checkLanguagesForDuplicates = (
    ...
    const languagesReducer = (
      state: membersTypes.LanguageOfMember[],
      action: membersTypes.languagesReducerAction
    ) => {
      switch (action.type) {
        case membersTypes.languagesReducerActionType
          .addNewLanguageWithValueAsString: {
        ...
        case membersTypes.languagesReducerActionType
          .addNewLanguageWithValueAsObject: {
          if (action.payload.lang.inputValue) {
            if (
              checkLanguagesForDuplicates(state, action.payload.lang.inputValue)
            ) {
              return state;
            }
            return updateLanguages(
              state,
              action.payload.index,
              action.payload.lang.inputValue,
              "WERT"
            );
     ...
     * @returns true it there would be a duplicate
     */
    const checkEdvSkillsForDuplicates = (
     ...
     */
    const edvSkillsReducer = (
      state: membersTypes.EDVSkillOfMember[],
      action: membersTypes.edvSkillsReducerAction
    ) => {
      switch (action.type) {
        case membersTypes.edvSkillsReducerActionType
          .addNewEdvSkillWithValueAsString: {
          if (checkEdvSkillsForDuplicates(state, action.payload.value)) {
            return state;
          }
          return updateEdvSkills(
            state,
            action.payload.index,
            action.payload.value,
            "WERT"
        ...
        case membersTypes.edvSkillsReducerActionType
          .addNewEdvSkillWithValueAsObject: {
          if (action.payload.edvSkill.inputValue) {
            if (
              checkEdvSkillsForDuplicates(
                state,
                action.payload.edvSkill.inputValue
              )
            ) {
              return state;
            }
            ...
            if (
              checkEdvSkillsForDuplicates(state, action.payload.edvSkill.wert)
            ) {
              return state;
            }
            return updateEdvSkills(
    ...
    const [languages, dispatchLanguages] = useReducer(
      languagesReducer,
      memberDetails.sprachen
    );
    const [edvSkills, dispatchEdvSkills] = useReducer(
      edvSkillsReducer,
      memberDetails.edvkenntnisse
    );
                ...
                <div>
                  {
                    ...
                    props.isOwner || permissionList.includes(1) ? (
                      <IconButton
                        onClick={(event) => handleGeneralInfoDialogOpen(event)}
                      >
        ...
        <div>
          {memberDetails.passiv_seit ? (
            <div>
              ...
              <Typography>
                {`Seit ${transformSQLStringToGermanDate(
                  memberDetails.passiv_seit
                )}`}
              </Typography>
            </div>
              ...
              <div className={classes.category}>
                <div className={classes.category}>
                  ...
                  <div className={classes.categoryItem}>
                    ...
                    memberDetails.mentor.vorname &&
                    memberDetails.mentor.nachname ? (
                      <NavLink
                ...
                <div className={classes.categoryItem}>
                  ...
                  <div className={classes.categoryItemList}>
                    {memberDetails.edvkenntnisse.map((edv) => {
                      return (
                        <Typography className={classes.categoryLine}>
                          {`${edv.wert}: ${getEDVNiveauLabel(
                            parseInt(edv.niveau, 10)
                          )}`}
                        </Typography>
                      );
                    })}
              ...
              <Grid container spacing={2}>
                <Grid item xs={12} sm={12} md={12} lg={12}>
                  <Autocomplete
                    ...
                    defaultValue={
                      departments.filter(
                        (dep) => dep.bezeichnung === department
                      )[0]
                    }
                ...
                <Grid item xs={12} sm={12} md={12} lg={12}>
                  <Autocomplete
                    ...
                    defaultValue={
                      members.filter(
                        (memb) => memb.mitgliedID === mentorState?.mitgliedID
                      )[0]
                    }
                          ...
                          >
                            ...
                            <Grid item xs={5}>
                              <FormControl className={classes.fullWidth}>
                                ...
                                >
                                  <MenuItem value={5}>
                                    {getLanguageNiveauLabel(5)}
                                  </MenuItem>
                                  <MenuItem value={4}>
                                    {getLanguageNiveauLabel(4)}
                                  </MenuItem>
                                  <MenuItem value={3}>
                                    {getLanguageNiveauLabel(3)}
                                  </MenuItem>
                                  <MenuItem value={2}>
                                    {getLanguageNiveauLabel(2)}
                                  </MenuItem>
                                  <MenuItem value={1}>
                                    {getLanguageNiveauLabel(1)}
                                  </MenuItem>
                                </Select>
                              </FormControl>
                            </Grid>
                      ...
                      >
                        <IconButton
                          ...
                          disabled={languages.some(
                            (lang) => lang.wert === "" || lang.niveau === ""
                          )}
                          onClick={() =>
                            dispatchLanguages({
                              type: membersTypes.languagesReducerActionType
                                .addEmptyLanguage,
                            })
                          }
                        >
                    ...
                    <Grid container spacing={1}>
                      ...
                      >
                        <IconButton
                          aria-label="add"
                          color="primary"
                          disabled={edvSkills.some(
                            (edv) => edv.wert === "" || edv.niveau === ""
                          )}
                          onClick={() => {
                            dispatchEdvSkills({
                              type: membersTypes.edvSkillsReducerActionType
                                .addEmptyEdvSkill,
                            });
                          }}
                        >
