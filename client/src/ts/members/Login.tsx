...
const Login: React.FunctionComponent = () => {
  ...
  function getPasswordField() {
    if (failedLogin) {
      return (
        <Textfield error className={classes.inputfield} id="password" label="Passwort" type="password" helperText='Passwort oder Benutzername sind nicht korrekt' value={password}
          onChange = {event => {setPassword(event.target.value)}} onKeyUp={handleKeyUp} fullWidth />
      );
    }
    return (
          ...
          <Paper className={classes.paper}>
            ...
            <form className={classes.login} onSubmit={event => {login()}}>
              <Textfield className={classes.inputfield} id="username" label="Benutzername" type="text" value={username}
                onChange = {event => {setUsername(event.target.value)}} onKeyUp={handleKeyUp} fullWidth />
              {getPasswordField()}
